...
public enum PlateTint implements StringRepresentable {
    ...
    public int getTintColor() { return this.baseTintColor; }
    public int getGlowTintColor() { return this.glowTintColor; }

    public static PlateTint fromDyeColor(DyeColor dyeColor) {
        if (dyeColor == null) return NONE;
        String dyeName = dyeColor.getSerializedName();
        for (PlateTint tint : values()) {
            if (tint.dyeColor == dyeColor || tint.name.equals(dyeName)) return tint;
        }
        return NONE;
    }
}
